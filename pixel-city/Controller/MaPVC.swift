...
class MapVC: UIViewController, UIGestureRecognizerDelegate {
    ...
    @objc func animateViewDown() {
        cancelAllSessions()
        pullUpViewHieghtConstraint.constant = 1
        UIView.animate(withDuration: 0.3) {
            self.view.layoutIfNeeded()
        }
    }
...
extension MapVC: MKMapViewDelegate {
    ...
    @objc func dropPin(sender: UITapGestureRecognizer){
        ...
        mapView.setRegion(coordinateRegion, animated: true)
        retieveUrls(annotation: annotation) { (finished) in
            if finished {
    ...
    func retieveUrls(annotation: DroppablePin, handler: @escaping (_ Status: Bool) -> ()) {
        Alamofire.request(flickerUrl(key: apiKey, annotation: annotation, number: 40)).responseJSON { (response) in
